...
# Enable TF32 mode for matrix multiplications - ensure speed + accuracy
...
def train_model(model, device, train_loader, validation_loader, accumulation_size, optimizer, scheduler, scaler, epochs, iou_threshold=0.5, confidence_threshold=0.5):
    ...
    for epoch in range(epochs):
        ...
        print(f'Epoch: {(epoch + 1)}/{epochs}, Training Loss: {np.mean(loss_sublist):.3f}, Validation Loss: {np.mean(cv_loss_list):.3f}, Elapsed Time: {end - start:.1f}s')
